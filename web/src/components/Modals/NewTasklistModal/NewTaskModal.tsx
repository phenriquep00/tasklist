...
export function NewTaskModal() {
    ...
    const handleCreateNewTasklist = async (event: FormEvent) => {
        ...
        if (createValidator) {
            const newUserTotalTasklists: any = await rewriteTasklists(dataFromForm.name);
            await supabase
                .from('user')
    ...
    return (
        <Dialog.Portal>
            ...
            <Dialog.Content className='flex flex-col items-center justify-between text-ctp-text bg-ctp-crust rounded-xl fixed top-1/2 left-1/2 w-auto h-auto gap-6 p-4 translate-x-[-50%] translate-y-[-50%]'>
                ...
                <Dialog.Description asChild>
                    <form action="" onSubmit={handleCreateNewTasklist}>
                        <div className='flex flex-row gap-2 items-center justify-center'>
                            <label className="text-ctp-text font-semibold text-lg" htmlFor="name">tasklist name: </label>
                            <input type="text" name="name" id="name" className='p-2 rounded bg-ctp-surface2' />

                            <button type="submit" className='flex items-center justify-center p-2 bg-ctp-green rounded font-medium text-ctp-crust hover:ring-2 ring-ctp-peach'>
                                {
                                    isLoading
                                        ?
